...
class MainActivity : Activity(), TextToSpeech.OnInitListener {
    ...
    private fun buildUi() {
        ...
        sphere = VoxelSphereView(this).apply { setOnClickListener { startListening() } }
    ...
    private fun showMenu() {
        ...
        android.app.AlertDialog.Builder(this).setTitle("LeeWay Pocket").setItems(items) { d, which ->
            when (which) {
                ...
                3 -> configureAutomationBridge()
                4 -> startListening()
                5 -> startActivity(Intent("android.media.action.IMAGE_CAPTURE"))
                else -> d.dismiss()
            }
        }.show()
    }
    ...
    private fun configureAutomationBridge() {
        ...
        android.app.AlertDialog.Builder(this)
            ...
            .setMessage("Use one canonical Pocket Lee webhook. You can change it later without rebuilding the app.")
            .setView(layout)
            .setPositiveButton("Save") { _, _ ->
                automation.configure(endpoint.text.toString(), token.text.toString())
                Toast.makeText(this, if (automation.isConfigured()) "Automation bridge saved." else "Endpoint not configured.", Toast.LENGTH_SHORT).show()
            }
    ...
    private fun startListening() {
        ...
        recognizer?.setRecognitionListener(object : RecognitionListener {
            ...
            override fun onPartialResults(partialResults: Bundle?) { if (tts.isSpeaking) tts.stop() }
        ...
        when {
            ...
            l.startsWith("email ") || l.startsWith("send an email") || l.startsWith("send email") -> {
                routeAutomation("email", q) {
                    say("The automation lane isn't connected yet, so I'm opening your email app instead.")
            ...
            l.startsWith("remind ") || l.contains(" reminder") -> {
                routeAutomation("reminder", q) {
                    say("The automation lane isn't connected yet. I saved that request in my notebook so we don't lose it.")
            ...
            l.startsWith("search ") || l.startsWith("research ") || l.startsWith("look up ") -> {
                routeAutomation("research", q) {
                    ...
                    say("I'm opening the research lane for $term.")
            ...
            l.contains("schedule") || l.contains("appointment") || l.contains("calendar") -> {
                routeAutomation("schedule", q) {
                    say("The automation lane isn't connected yet, so I'm handing this to your calendar.")
                    startActivity(
            ...
            l.contains("who are you") -> say("I'm Lee. Pocket edition. Same LeeWay bloodline, built light enough to ride with you.")
            else -> routeAutomation("assistant_request", q) {
                say("I heard you. The Pocket Harness is live. The reasoning and neural voice modules are still being wired in.")
            }
        }
    }
    ...
    private fun routeAutomation(action: String, request: String, fallback: () -> Unit) {
        ...
        sphere.state = VoxelSphereView.State.THINKING
